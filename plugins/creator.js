...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨wahyudi.my.id@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://s.id/WahyudiID`, `Chindy Wening Mustika`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://s.id/WahyudiID`, https://s.id/WahyudiID`🤖 Dont Forget To Donate!`]
  ], fkontak)
